//! Direct UV-5R programming over a serial cable.
//!
//! This module speaks the Baofeng UV-5R clone protocol — the same one CHIRP's
//! `chirp/drivers/uv5r.py` implements — so 73plug can read from and (later)
//! write to the radio without going through CHIRP.
//!
//! **Phase A (this file): read-only.** Port enumeration, radio identify, and a
//! faithful full-image download that is saved as a CHIRP-compatible `.img`
//! backup. There is intentionally NO write/upload path here yet — writing the
//! radio's EEPROM can brick it, so the read path is proven against real
//! hardware first (see PROJECT_STATE / the plan).
//!
...
use crate::commands::program_settings;
...
const CHANNEL_ADDR: (u16, u16) = (0x0000, 0x0800);
const NAME_ADDR: (u16, u16) = (0x1000, 0x1800);
...
fn backup_filename(kind: &str, label: &str, stamp: &impl std::fmt::Display) -> String {
...
/// safety backup that the (future) write path will always take first.
#[tauri::command]
...
/// 73plug profile (the inverse of writing settings during programming). The full
...
pub async fn read_radio_settings(
    ...
    std::fs::create_dir_all(&backup_dir).estr()?;
    let stamp = chrono::Local::now().format("%Y%m%d-%H%M%S");
    let backup_path = backup_dir.join(backup_filename("settings", &profile_name, &stamp));
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let settings = program_settings::decode_settings_from_image(&image, &schema)?;
...
fn ident_radio(p: &mut dyn SerialPort) -> Result<(String, Vec<u8>), String> {
...
fn reident_with_retry(p: &mut dyn SerialPort) -> Result<(String, Vec<u8>), String> {
...
fn decode_channels(image: &[u8]) -> Vec<DecodedChannel> {
    let mut out = Vec::new();
...
pub async fn program_codeplug(
    ...
    std::fs::create_dir_all(&backup_dir).estr()?;
    let stamp = chrono::Local::now().format("%Y%m%d-%H%M%S");
    let backup_path = backup_dir.join(backup_filename("prewrite", &codeplug_name, &stamp));
    ...
    let result = tauri::async_runtime::spawn_blocking(move || -> Result<ProgramResult, String> {
        ...
        let settings_written = match (&settings_schema, &profile_settings) {
            (Some(schema), Some(settings)) => {
                Some(program_settings::apply_profile_settings(&mut image, schema, settings)?)
            }
            _ => None,
        };
        ...
        if write_settings {
            for &(start, end) in program_settings::SETTINGS_MAIN_RANGES {
                write_region(&mut *p, &image, start, end)?;
            }
            for &(start, end) in program_settings::SETTINGS_AUX_RANGES {
...
/// must be a 73plug/CHIRP UV-5R image (it carries the 8-byte ident prefix, so
/// radio address 0x0000 is at image offset 0x0008).
#[tauri::command]
pub async fn restore_image(port: String, path: String) -> Result<RestoreResult, String> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        // Aux ranges only if the backup actually contains the aux block.
        let aux_end = program_settings::AUX_IMAGE_BASE
            + (0x1FD0 - program_settings::AUX_RADIO_BASE as usize);
...
fn write_region(p: &mut dyn SerialPort, image: &[u8], start: u16, end: u16) -> Result<(), String> {
    let mut addr = start;
    while addr < end {
...
fn write_aux_region(
    ...
    let mut addr = start;
    while addr < end {
        let off = program_settings::AUX_IMAGE_BASE
            + (addr - program_settings::AUX_RADIO_BASE) as usize;
