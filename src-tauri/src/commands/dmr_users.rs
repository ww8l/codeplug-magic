...
mod tests {
    ...
    fn real_radioid_sample_deserializes() {
        let json = r#"{"users":[{"name":"Hans Juergen","fname":"Hans Juergen","surname":"",
            "country":"Canada","callsign":"VA3BOC","city":"Cornwall","radio_id":1023007,
            "id":1023007,"state":"Ontario","has_valid_callsign":"1","has_valid_email":0,
            "account_unvalidated":0,"account_validation_status":"validated"}]}"#;
        let dump: RadioIdDump = serde_json::from_str(json).unwrap();
        assert_eq!(dump.users.len(), 1);
        let u = &dump.users[0];
        assert_eq!(u.id, 1023007);
        assert_eq!(u.callsign, "VA3BOC");
        assert_eq!(u.city.as_deref(), Some("Cornwall"));
        assert_eq!(u.state.as_deref(), Some("Ontario"));
        assert_eq!(u.country.as_deref(), Some("Canada"));
