...
const PROBE_REGIONS: &[(&str, u32, u32)] = &[
...
const DUMP_REGIONS: &[(&str, u32, u32)] = &[
...
/// Scan lists, from the dmr-tools `d890uv` map (descriptor-sourced; layout not
/// yet HW-verified — the program flow's Verify step covers it). 250 records of
/// 0x200 bytes at `SCAN_LIST_BASE`: unused(1) + priority enum(1) + primary/
/// secondary priority channel index (u16 LE, idx+1, 0=selected, 0xFFFF=none) +
/// four u16 LE timers (0.1 s units) + 16-char UTF-16 name @0x0E + 2 unused +
/// 100 × u16 LE 0-based channel indices @0x30 (0xFFFF = unset) + revert enum
/// @0xF8 + zero-filled tail.
...
pub(crate) const SCAN_MAX_CHANNELS: usize = 100;
...
const CONTACT_CALL_TYPE: usize = 0x00;
const CONTACT_DMR_ID: usize = 0x02;
const CONTACT_NAME: usize = 0x06;
...
fn try_enter_program_and_ident(p: &mut dyn SerialPort) -> Result<Vec<u8>, String> {
...
fn ident_is_d890(ident: &[u8]) -> bool {
...
fn read_channel_banks(p: &mut dyn SerialPort) -> (Vec<RegionProbe>, Vec<(usize, Vec<u8>)>) {
...
fn identify_checksum(addr: u32, size: u8, data: &[u8], raw: u8) -> String {
    let a = addr.to_be_bytes();
...
/// unit-testable.
fn validate_read_header(addr: u32, size: u8, header: &[u8]) -> Result<(), String> {
    let a = addr.to_be_bytes();
...
fn anytone_checksum(addr: &[u8; 4], data: &[u8]) -> u8 {
...
fn build_write_frame(addr: u32, data: &[u8]) -> Vec<u8> {
...
fn read_record(p: &mut dyn SerialPort, addr: u32) -> Result<Vec<u8>, String> {
...
fn bank_of_slot(slot: usize) -> (u32, usize, usize) {
    let zero = slot - 1;
    let bank = zero / CH_PER_BANK;
    let within = zero % CH_PER_BANK;
...
const CH_RX: usize = 0x00;
const CH_TX_OFFSET: usize = 0x04;
const CH_MODE: usize = 0x08;
const CH_TONE_EN: usize = 0x09;
const CH_TX_CTCSS: usize = 0x0A;
const CH_RX_CTCSS: usize = 0x0B;
const CH_TX_DCS: usize = 0x0C;
const CH_RX_DCS: usize = 0x0E;
const CH_CONTACT: usize = 0x14;
const CH_COLOR_CODE: usize = 0x20;
const CH_TIME_SLOT: usize = 0x21;
const CH_NAME: usize = 0x44;
const CH_NAME_CHARS: usize = 16;
...
/// transmit on CC 0.
const CH_TX_COLOR_CODE: usize = 0x43;
...
fn encode_mode_byte(shift: u8, wide: bool, power: u8, mode: u8) -> u8 {
    ((shift & 0x03) << 6) | ((wide as u8) << 4) | ((power & 0x03) << 2) | (mode & 0x03)
}

/// Decode the 16-char UTF-16LE name field.
fn decode_name(rec: &[u8]) -> String {
...
fn power_label(mode_byte: u8) -> &'static str {
...
fn bandwidth_label(mode_byte: u8) -> &'static str {
...
fn channel_mode(mode_byte: u8) -> (&'static str, bool) {
...
fn dcs_label(rec: &[u8], off: usize) -> String {
...
fn tone_side(ctcss_on: bool, dcs_on: bool, rec: &[u8], ctcss_off: usize, dcs_off: usize) -> Option<String> {
    if ctcss_on {
...
fn decode_tone_sides(rec: &[u8]) -> (Option<AnytoneSubTone>, Option<AnytoneSubTone>) {
...
fn decode_channel(rec: &[u8], slot: usize) -> Option<AnytoneDecodedChannel> {
...
fn decode_channels(banks: &[(usize, &[u8])]) -> (Vec<AnytoneDecodedChannel>, HashMap<usize, String>) {
...
fn encode_shift(rx_mhz: f64, tx_mhz: f64) -> (u8, [u8; 4]) {
...
fn encode_name_into(field: &mut [u8], name: &str) {
...
fn encode_tone_into(rec: &mut [u8], tone: &AnytoneToneEdit) -> Result<(), String> {
...
///
/// NOTE: the `priority_select` (@0x01) and `revert_channel` (@0xF8) enum byte
/// values are sourced from qdmr and are NOT yet hardware-verified — pin them
/// with the re_settings_diff differential-dump loop before trusting them.
pub(crate) struct ScanListRecordSettings {
...
fn priority_index(slot: Option<u16>) -> u16 {
    slot.map(|s| s + 1).unwrap_or(0xFFFF)
}

pub(crate) fn encode_scan_list_record(
    name: &str,
    indices: &[u16],
    ...
    // Revert Channel Type @0xF8; tail stays zero.
    out[0xF8] = settings.revert_channel;
    Ok(out)
}
...
mod tests {
    ...
    fn scan_list_record_layout_and_rejections() {
        ...
        assert_eq!(def[0xF8], 0); // revert = Selected Channel
        assert!(def[0xF9..].iter().all(|&b| b == 0));
