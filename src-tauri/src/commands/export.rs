...
pub async fn generate_codeplug(
    ...
    match model.export_format.as_deref() {
        ...
        Some("anytone_csv") => write_anytone_bundle(&path, &included, &model)?,
...
fn anytone_call_type(call_type: Option<&str>) -> &'static str {
    match call_type {
...
fn contact_name(label: &str) -> String {
...
fn render_anytone_talkgroups(channels: &[&ExpandedChannel]) -> Result<String, String> {
    let mut wtr = csv::Writer::from_writer(vec![]);
